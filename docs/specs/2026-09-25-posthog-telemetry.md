...
9. Watch: workout_started is never sent for watch workouts (no SDK on the watch); workout_completed for a watch workout is sent once by the iPhone, including the CloudKit-first path, deduplicated by workout id.
10. iOS crash reports produced while the switch was off are purged before the SDK is set up again; first-launch crash capture waits for the remote configuration to be cached (documented limit).
