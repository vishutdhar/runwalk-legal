...
5. `SettingsTabView.swift`: a new section "Privacy" with the toggle "Share anonymous usage and crash reports" and a one-line footer: "Helps fix crashes and improve Canter. Sends coarse workout numbers such as duration and distance, never your routes, GPS positions or health data." (wording amended, see section 8) Wire it to `setEnabled`.
...
## 8. Amendments made during implementation (September 25, 2026; these override the sections above where they differ)

1. Identifier: the PostHog iOS 3.58.3 identifier is a random UUID generated and stored by the SDK, not identifierForVendor; the SDK never reads the vendor identifier, and its storage is excluded from backups. Section 3 item 7's IDFV wording is withdrawn. The Android identifier is likewise SDK-generated (not ANDROID_ID, not the advertising id).
2. distance_m carries the App's own GPS route distance only; 0 when GPS was off or there is no route. HealthKit or Health Connect distance never reaches an event.
3. Switch-off sends exactly one event, telemetry_disabled, then flushes and opts out; setting_changed is NOT sent for the telemetry key. Switch-on in the same process opts back in (iOS) or sets the SDK up again (Android) and sends nothing extra; a launch with the choice off never sets the SDK up.
4. Retention: the organization is on PostHog's paid plan, so the published policy states a seven year query window (not one year) and that retention is not a deletion guarantee. Section 5's one-year text is withdrawn.
5. Strava events are wired on iOS but cannot fire (StravaConfig.serviceAvailable is false in the live build) and do not exist on Android; the published policy does not list them until Strava ships.
6. Settings footer copy on both platforms: "Helps fix crashes and improve Canter. Sends coarse workout numbers such as duration and distance, never your routes, GPS positions or health data."
7. Store declarations: workout duration, distance, interval count and plan identifiers are fitness data. iOS PrivacyInfo.xcprivacy declares NSPrivacyCollectedDataTypeFitness (not linked, not tracking); Play Data Safety carries a "Health and fitness > Fitness info" row (collected, optional, not ephemeral, not shared, analytics). PostHog is a service provider, so Play "Shared" is No.
8. Surveys are disabled in the SDK config on both platforms in phase 1; GeoIP enrichment is disabled on the PostHog project; IP discard is on; deep-link, rage-click, dead-click and element autocapture are off; screen views and lifecycle events are on; feature flags are not preloaded.
9. Watch: workout_started is never sent for watch workouts (no SDK on the watch); workout_completed for a watch workout is sent once by the iPhone, including the CloudKit-first path. Every completion on both platforms (live save, a workout restored at launch after process death or a failed save, a watch handoff) goes through one ledger keyed by the record id, so each workout is reported at most once; a restore reports the workout's remembered start context (source, program, auto_paused) and falls back to phone/false when none was kept. A completion that happens while the switch is off is ledgered and never sent later. Section 2's "restored workouts are not reported" reading is withdrawn.
10. iOS crash reports produced while the switch was off are purged before the SDK is set up again; first-launch crash capture waits for the remote configuration to be cached (documented limit); after an off then on inside one process, crash capture resumes at the next launch (PLCrashReporter cannot be re-installed in-process). Switch-off on both platforms: capture telemetry_disabled, flush until the SDK queue is empty or 5 s pass, close, delete the SDK queue files, so nothing captured before the switch-off is sent later.
